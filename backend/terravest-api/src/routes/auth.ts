...
// --- EMAIL HELPERS ---
function getVerificationEmailContent(lang: Lang, url: string) {
    const templates: Record<string, { subject: string, body: string, btn: string }> = {
        'en': { subject: 'Verify your TerraVest email', body: 'Welcome to TerraVest! Please verify your email address to activate your account.', btn: 'Verify Email' },
        'pt-BR': { subject: 'Verifique seu e-mail da TerraVest', body: 'Bem-vindo ao TerraVest! Verifique seu endereço de e-mail para ativar sua conta.', btn: 'Verificar E-mail' },
        'es-419': { subject: 'Verifica tu correo de TerraVest', body: '¡Bienvenido a TerraVest! Verifica tu dirección de correo para activar tu cuenta.', btn: 'Verificar Correo' },
        'fr': { subject: 'Vérifiez votre e-mail TerraVest', body: 'Bienvenue sur TerraVest ! Veuillez vérifier votre adresse e-mail pour activer votre compte.', btn: 'Vérifier l\'e-mail' }
    };
    ...
    const html = `
        <div style="font-family: sans-serif; padding: 20px;">
            <h2>${t.subject}</h2>
            <p>${t.body}</p>
            <a href="${url}" style="display: inline-block; background-color: #009B9E; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; margin-top: 10px;">${t.btn}</a>
            <p style="margin-top: 20px; font-size: 12px; color: #666;">Or paste this link: ${url}</p>
        </div>
    `;
    return { subject: t.subject, html };
}

function getResetEmailContent(lang: Lang, url: string) {
    const templates: Record<string, { subject: string, body: string, btn: string }> = {
        'en': { subject: 'Reset your Password', body: 'We received a request to reset your password. Click the button below to proceed.', btn: 'Reset Password' },
        'pt-BR': { subject: 'Redefinir sua Senha', body: 'Recebemos uma solicitação para redefinir sua senha. Clique no botão abaixo para prosseguir.', btn: 'Redefinir Senha' },
        'es-419': { subject: 'Restablecer tu Contraseña', body: 'Recibimos una solicitud para restablecer tu contraseña. Haz clic en el botón de abajo para continuar.', btn: 'Restablecer Contraseña' },
        'fr': { subject: 'Réinitialiser votre Mot de Passe', body: 'Nous avons reçu une demande de réinitialisation de votre mot de passe. Cliquez ci-dessous pour continuer.', btn: 'Réinitialiser' }
    };
    const t = templates[lang] || templates['en'];
    ...
    const html = `
        <div style="font-family: sans-serif; padding: 20px;">
            <h2>${t.subject}</h2>
            <p>${t.body}</p>
            <a href="${url}" style="display: inline-block; background-color: #0F172A; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold; margin-top: 10px;">${t.btn}</a>
...
export async function handleRegister(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        try {
            ...
            const frontendUrl = env.FRONTEND_URL || 'https://terravest.homes';
...
export async function handleLogin(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        // Identifier-aware authentication
        let user: any;
        const query = identifierType === "email"
            ? 'SELECT id, email, username, password, role, email_verified FROM users WHERE LOWER(email) = ?'
            : 'SELECT id, email, username, password, role, email_verified FROM users WHERE LOWER(username) = ?';

        user = await env.terravest_db.prepare(query).bind(normalizedIdentifier).first();
...
export async function handleVerifyEmail(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        await db.batch([
            db.prepare("UPDATE users SET email_verified = 1, email_verified_at = CURRENT_TIMESTAMP WHERE id = ?").bind(matched.user_id),
            db.prepare("UPDATE email_verification_tokens SET used = 1 WHERE id = ?").bind(matched.id)
        ]);
...
export async function handleCheckUsername(request: Request, env: Env): Promise<Response> {
    try {
        ...
        if (!username) return json({ available: false, reason: "username_required" }, 400);

        const normalizedUsername = username.toLowerCase();

        if (!USERNAME_REGEX.test(username)) return json({ available: false, reason: "invalid_format" });
        if (isReservedUsername(normalizedUsername)) return json({ available: false, reason: "reserved" });
...
export async function handleCheckEmail(request: Request, env: Env): Promise<Response> {
    try {
        ...
        if (!email) return json({ available: false, reason: "email_required" }, 400);

        const normalizedEmail = email.toLowerCase();
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) return json({ available: false, reason: "invalid_email" });
...
export async function handleForgotPassword(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    try {
        ...
        if (env.TURNSTILE_SECRET) {
            if (!turnstileToken) return errorResponse(getErrorMessage(lang, 'BOT_VERIFICATION_FAILED'), 400);
            const isHuman = await verifyTurnstile(turnstileToken, env.TURNSTILE_SECRET, ip);
            if (!isHuman) return errorResponse(getErrorMessage(lang, 'BOT_VERIFICATION_FAILED'), 403);
        }
...
export async function handleResetPassword(request: Request, env: Env): Promise<Response> {
    try {
        ...
        const tokens = await env.terravest_db
            .prepare(`SELECT id, user_id, token_hash FROM password_reset_tokens WHERE used = 0 AND expires_at > datetime('now')`)
            .all();
        ...
        await env.terravest_db.batch([
            env.terravest_db.prepare('UPDATE users SET password = ? WHERE id = ?').bind(hashedPassword, validToken.user_id),
            env.terravest_db.prepare('UPDATE password_reset_tokens SET used = 1 WHERE id = ?').bind(validToken.id)
        ]);
...
export async function handleMe(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        return json({
            ...
            username: user.username,
            role: user.role,
            usd_balance: Number(user.usd_balance || 0)
        });
